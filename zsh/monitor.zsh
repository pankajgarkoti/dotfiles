...
status() {
	if grep -q "size = 17" "$ALACRITTY_CONFIG"; then
		echo "Current mode: Laptop (font size 17)"
...
# Switches between:
#   - Font size 17 with commented background (laptop mode)
...
toggle() {
	if grep -q "size = 17" "$ALACRITTY_CONFIG"; then
		...
		sed -i '' 's/size = 17/size = 20/' "$ALACRITTY_CONFIG"
		...
		# Change font size to 17 and comment background
		sed -i '' 's/size = 20/size = 17/' "$ALACRITTY_CONFIG"
		sed -i '' "s/background = '#000000'/# background = '#000000'/" "$ALACRITTY_CONFIG"
		echo "Switched to: Laptop mode (font size 17, theme default background)"
	fi
}
...
# Switches between font size 17 (laptop) and 20 (external monitor)
...
toggle_font() {
	if [ -n "$1" ]; then
		...
		if grep -q "size = 17" "$ALACRITTY_CONFIG"; then
			...
			sed -i '' 's/size = 20/size = 17/' "$ALACRITTY_CONFIG"
			echo "Switched to: Font size 17 (Laptop)"
		fi
	fi
}
